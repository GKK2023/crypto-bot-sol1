...
class SimpleBot:
    def __init__(self):
        ...
        if PAPER_MODE:
            ...
            self.balance = {'USDT': 10000, 'SOL': 0}; self.position = None
        else:
            ...
            self.exchange.fetch_time(); print("Connexion à Gate.io SOL réussie!")
    ...
    def _detect_existing_position(self):
        ...
        if sol_balance < MIN_POSITION_THRESHOLD: print(f"Pas de position SOL"); self.position = None; return
        ...
        if entry_price:
            ...
            print(f"Position SOL: {sol_balance:.4f} @ ${entry_price:.4f}")
    ...
    def get_entry_price_from_orders(self):
        try:
            ...
            if buy_orders:
                ...
                if price and float(price) > 0: print(f"[DEBUG] Prix achat (orders): ${float(price):.4f}"); return float(price)
            return None
    ...
    def get_entry_price_from_trades(self):
        try:
            ...
            if price and float(price) > 0 and float(cost) > 5:
                print(f"[DEBUG] Prix achat (trades): ${float(price):.4f} | Cost: ${float(cost):.2f}"); return float(price)
            return None
        except Exception as e: print(f"[DEBUG] Erreur: {e}"); return None
    ...
    def get_real_balance(self):
        try:
            ...
            if isinstance(balance, dict):
                ...
                if isinstance(total, dict): return {'USDT': float(total.get('USDT', 0) or 0), 'SOL': float(total.get('SOL', 0) or 0)}
            return {'USDT': 0, 'SOL': 0}
        except: return {'USDT': 0, 'SOL': 0}
    ...
    def calculate_macd(self, data):
        try:
            ...
            closes = data['close'].values
            ema12 = self._ema(closes, 12); ema26 = self._ema(closes, 26)
            macd = ema12 - ema26
    ...
    def should_buy(self, data):
        ...
        if MACD_CONFIRM and not macd_improving:
            print(f"  -> RSI OK ({rsi:.1f} < {RSI_BUY_THRESHOLD}) mais MACD se dégrade (histogramme: {histogram:.3f})")
            return False
        return True
    ...
    def sell(self):
        try:
            self.balance = self.get_real_balance(); sol_balance = float(self.balance.get('SOL', 0))
            if sol_balance < MIN_POSITION_THRESHOLD: self.position = None; return
            ...
            if PAPER_MODE:
                ...
                print(f"  VENTE RÉELLE: {amount:.4f} SOL @ ${price}"); self.position = None
            ...
            try:
                ...
                if data is not None:
                    ...
                    if price is not None:
                        ...
                        usdt_bal = float(self.balance.get('USDT', 0)); sol_bal = float(self.balance.get('SOL', 0))
                        rsi, macd, signal, histogram, macd_improving = self.get_indicators(data)
                        ...
                        if self.position is None:
                            ...
                            if sol_bal < MIN_POSITION_THRESHOLD: self.position = None
